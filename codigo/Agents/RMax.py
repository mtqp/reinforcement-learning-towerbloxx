...
class RMax(object):
  ...
  def choose_action(self, state):
        ...
        for action in [0,1]: #actions
          ...
          for possible in self.reachable_states: #todos los estados que se que existen
            if values.get(possible) is None:
              valueToUse = 0.0 if possible in self.visited_states else self.vmax #si no esta, uso vmax
            else:
            ...
            partial_Q += RMAX_GAMMA_VALUE_ITER * self.T_value(action, visited, possible) * valueToUse
    ...
    return qMax#random.choice([a for a in [0,1] if (Q.get((a,state))or 0.0)==qMax])
